...
/**
 * A stable client-side identity, generated once per row and never sent.
 *
 * The builder needs a key that survives a whole-array `replace()`, which is how
 * every structural edit is applied now that a part is one flat list spanning
 * several wire groups. `useFieldArray`'s own `field.id` cannot do that job — it
 * is regenerated by `replace`, so React would remount every row and the drag
 * that caused the move would end on a different element. `serverId` cannot
 * either: a row that has never been saved does not have one.
 */
const formUid = z.string().min(1);
...
/**
 * A wire group. **The teacher never sees one.**
 *
 * `TestQuestionGroup` is how the backend shares one passage between several
 * questions, and it stays exactly as it is on the wire. In the builder it has
 * no chrome of its own: a part renders as one flat numbered list, and a group
 * shows up only as the shared material sitting above the questions it covers.
 * See `docs/features/test-studio.md`, "No groups".
 */
export const groupFormSchema = z.object({
  ...
  imageUrl: z.string(),
  /**
   * Whether this group carries shared material the questions under it depend
   * on. UI-only, and deliberately not derived from the content: a passage the
   * teacher has just added but not yet typed into is empty, and a card that
   * vanished the moment it was created would be unusable. Derived from the
   * content only when seeding the form from the server.
   */
...
export const sectionFormSchema = z.object({
  uid: formUid,
  serverId: z.string(),
  title: z.string().trim().min(1, "Name this part.").max(TEST_LIMITS.titleChars),
  instructions: z.string().max(TEST_LIMITS.instructionsChars),
  kind: z.string().min(1),
